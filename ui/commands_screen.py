...
COMMAND_OPTIONS: List[Tuple[str, str]] = [
    ...
    ("/memory", "память агента: слои, записи, управление"),
    ("/profile", "профиль пользователя: настройка вопрос-ответ, переключение"),
    ("/task", "задача агента: очередь, состояние, прогон, переходы"),
    ("/branches", "ветки диалога: переключить, чекпоинт, новая ветка"),
    ("/invariants", "инварианты агента: правила, проверка ответа, отказ"),
    ("/mcp", "подключение к MCP-серверу: сервер, протокол, инструменты"),
    ("/tool", "инструменты MCP: перечень, ручной вызов, автовызов, последний флоу"),
    ("/schedule", "планировщик: задания, расписание, прогоны, накопленное"),
]
