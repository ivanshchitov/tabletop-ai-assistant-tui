...
# openspec/changes/archive/2026-09-04-add-model-usage-metadata/design.md); прежняя оговорка про
# пиковые и непиковые цены снята вместе с обновлением пула — прайс публикует одно число на модель.
MODEL_PRICING = {
...
TOKENS_OVERHEAD = 50
# Нижний порог того же потолка. Reasoning-модели пула тратят сотни токенов на рассуждение
# *до* первого символа ответа: измерено на deepseek-v4.1-flash — при max_tokens=170 (потолок
# 30-словного ответа) приходит пустой content с finish_reason=length и 470 токенов рассуждения,
# при 500 — нормальный ответ. Потолок технический, а длину задаёт инструкция промпта, поэтому
# порог ничего не ломает: он лишь не даёт формуле опуститься ниже того, что модели нужно на
# разгон. Вспомогательные запросы (суммаризатор, факты, конвейер) считают потолок той же
# функцией и получают тот же запас.
MIN_REQUEST_MAX_TOKENS = 2000


def max_tokens_for_words(max_words: int) -> int:
    return max(max_words * WORDS_TO_TOKENS_RATIO + TOKENS_OVERHEAD, MIN_REQUEST_MAX_TOKENS)
...
# перекрывали наблюдённый разброс — та же причина, по которой MAX_MAX_WORDS подняли до 1000.
# День 20: просьба на длинный флоу («подбери игру, дай факты, правила, сводку, сохрани») съела
# на deepseek-v4.1-flash весь потолок в 2000 токенов reasoning-ом (finish_reason=length, пустой
# content), а раунд, где модель пишет JSON правил аргументом (рулбук ~33 тыс. символов на входе),
# не уложился и в 4050 — снова пустой content; 8050 хватило с запасом (1617 токенов вывода).
# 2000 слов — для всех раундов: потолок ничего не стоит, платится только выданное.
TOOL_CHOICE_MAX_WORDS = 2000
# Цепочка инструментов (день 19): модель одним запросом выбора отдаёт до стольких шагов.
# Лишние отбрасываются, а не выполняются: «получить → обработать → сохранить» укладывается в
# три шага, запас в один — на уточняющий шаг, дальше цепочка тянула бы время и контекст.
TOOL_CHAIN_MAX_STEPS = 4
# Флоу из раундов (день 20): ответ выбора с "more": true получает следующий раунд — новый запрос
# выбора с результатами выполненных шагов. Пределы держат длинный флоу конечным: раундов и шагов
# на вопрос. Результаты шагов в запросе раунда — в общем бюджете символов (рулбук в ~20 тыс.
# символов должен дойти целиком, когда модель извлекает из него правила).
TOOL_FLOW_MAX_ROUNDS = 6
TOOL_FLOW_MAX_STEPS = 10
TOOL_FLOW_CONTEXT_CHARS = 24000
# Результаты прошлых раундов — справка, а не рабочий материал: живой прогон с рулбуком в
# 33 тыс. символов нёс его в каждый следующий раунд, и reasoning съедал бюджет выбора.
# Шаги последнего раунда идут целиком (в общем бюджете), шаги прошлых — не длиннее этого.
TOOL_FLOW_OLD_RESULT_CHARS = 1500
# Результат шага в запросе *ответа*: сырой рулбук в 33 тыс. символов, оставшийся после оборванного
# флоу, съел reasoning-ом весь потолок ответа (пустой content). Полный текст по-прежнему уходит в
# аргументы следующих шагов через $N — предел касается только сообщения модели ответа.
TOOL_ANSWER_RESULT_CHARS = 6000
...
# Каталог выгрузок цепочки инструментов (день 19): туда пишет инструмент сохранения
# собственного сервера. Пишет серверный процесс, поэтому переменная — в env_keys его записи
# реестра; читается в момент вызова, как адрес внешнего API, чтобы подмена в тесте действовала.
DEFAULT_EXPORTS_DIR = BASE_DIR / "exports"


def exports_dir() -> Path:
    """Каталог выгрузок: TABLETOP_EXPORTS_DIR или exports/ в корне репозитория."""
    return Path(os.getenv("TABLETOP_EXPORTS_DIR", str(DEFAULT_EXPORTS_DIR)))
...
MCP_SERVERS = {
    ...
    "dnd-rules": MCPServerSpec(
        ...
        description=(
            ...
            "планировщик отложенных и периодических вызовов; сводка и сохранение в файл"
        ),
    ),
