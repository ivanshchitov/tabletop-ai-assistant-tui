...
- `.claude/skills/feature-builder/` (mirrored at `.opencode/skills/feature-builder/`) is
  ...
  plan — no `writing-plans` duplicate) → a plain branch named after the change (`git switch -c`,
  never a worktree: the e2e layer runs the app from the repo root) → `/opsx:apply` with per-task
  TDD (failing test, minimal code, `pytest`) → final `pytest` + `/opsx:archive <change-id>` +
  `CLAUDE.md`/`README.md` update + fast-forward merge to `main`. Every git mutation waits for an
  explicit "yes" after `git status`/`git diff --stat`. Runs caveman-style throughout — status
  lines only, no code narration.
