...
revises a change's artifacts, `/opsx:sync` pushes deltas into the main specs without archiving.
...
- `openspec/config.yaml` sets the artifact language to Russian (structural headings and SHALL/MUST
  stay English) to match `README.md`. That file, not the generated skills in `.claude/skills/`, is
  where per-artifact rules and per-operation guidance belong — the skills are regenerated by
  `openspec init` and hand edits there are lost.
